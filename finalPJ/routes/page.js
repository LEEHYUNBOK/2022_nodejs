...
const { Post, User, Hashtag, Choose } = require('../models')
    ...
    let posts = await Post.findAll({
      include: [
        ...
        },
        {
          model: Choose,
          // id, nick 으로 출력
          attributes: ['choose', 'UserId', 'PostId'],
        },
      ],
      // 날짜 순으로 불러오기
      ...
      for (let i = 0; i < data.length; i++) {
        let count1 = 0
        let count2 = 0
        for (let l = 0; l < data[i].Chooses.length; l++) {
          if (data[i].Chooses[l].choose === 1) count1++
          else count2++
        }
        data[i].resultChoose1 = count1
        data[i].resultChoose2 = count2
      }
      // console.log(data)
      return data
    })
